...
let getDetailsFromNuGetViaODataFast auth nugetURL (packageName:PackageName) (version:SemVerInfo) =
    async {
        ...
        let fallback6 () =
            async {
                let url = sprintf "%s/Packages(Id='%s',Version='%O')" nugetURL (packageName.ToString()) normalizedVersion
                let! raw = getFromUrl(auth,url,acceptXml)
                if verbose then
        ...
        let fallback5 () =
            async {
                let url = sprintf "%s/Packages(Id='%s',Version='%O')" nugetURL (packageName.ToString()) version
                let! raw = getFromUrl(auth,url,acceptXml)
                if verbose then
                    tracefn "Response from %s:" url
                    tracefn ""
